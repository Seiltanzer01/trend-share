...
def create_predefined_data():
    ...
    # Создаём категории критериев, подкатегории и критерии
    # (Повторение возможно — убедитесь, что это необходимо)
    # ...
...
# Добавление обработчиков команд перед построением приложения
builder.add_handler(CommandHandler('start', start))
builder.add_handler(CommandHandler('help', help_command))
builder.add_handler(CommandHandler('add_trade', add_trade_command))
builder.add_handler(CommandHandler('view_trades', view_trades_command))
builder.add_handler(CommandHandler('register', register_command))
