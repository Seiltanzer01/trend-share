...
STAKING_CONTRACT_ADDRESS = os.environ.get('STAKING_CONTRACT_ADDRESS', '') 
TOKEN_DECIMALS = int(os.environ.get('TOKEN_DECIMALS', '18'))

# Обновлённый ABI (не забудьте вставить тот, что вы получили при компиляции)
...
def init_staking_web3():
    ...
    if BASE_RPC_URL and PRIVATE_KEY and STAKING_CONTRACT_ADDRESS:
        ...
        logger.error("Not all env vars (BASE_RPC_URL, PRIVATE_KEY, STAKING_CONTRACT_ADDRESS) are set.")

def stake_tokens_from_backend(amount_float: float):
    """
    Подпись транзакции от owner_account => stake(amount).
    amount_float в “токенах”.
    """
...
def unstake_from_backend():
    """
    Вызываем unstake() от owner_account (по аналогии).
    """
    if not web3 or not staking_contract or not owner_account:
        return False, "Staking contract not init"
    try:
