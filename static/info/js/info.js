...
/* Initialize Dexscreener Chart using Chart.js */
async function initDexChart() {
  const ctx = document.getElementById('dexChart').getContext('2d');
  try {
    const response = await fetch('https://api.dexscreener.com/latest/dex/pairs/base/0x70560C2cd364BA7fb197B9c4de021F8C34E9C8ed');
    const data = await response.json();
    const chartData = data.pairs[0].chartData || [];
    ...
    if (window.dexChartInstance) {
      window.dexChartInstance.data.labels = labels;
      window.dexChartInstance.data.datasets[0].data = prices;
      window.dexChartInstance.update();
    } else {
      window.dexChartInstance = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            label: 'Price (USD)',
            data: prices,
            borderColor: 'rgba(243, 57, 18, 1)',
            backgroundColor: 'rgba(243, 57, 18, 0.2)',
            fill: true
          }]
        },
        options: {
    ...
    console.error('Ошибка получения данных Dexscreener:', error);
  }
}
initDexChart();
setInterval(initDexChart, 60000); // обновление каждую минуту
